...
async function loadImages() {
  const response = await fetch("./assets/images/data.json");
  ...
  images.forEach(img => {
    ...
    wrapper.innerHTML = `
      <div class="relative overflow-hidden rounded-xl group">
        <img src="./assets/images/${img.src}" class="w-full transition-transform duration-300 group-hover:scale-110" />
      ...
      <div class="flex justify-between items-center p-2 text-sm">
        <span>${img.name}</span>
        <a href="./assets/images/${img.name}" download class="cursor-pointer">
